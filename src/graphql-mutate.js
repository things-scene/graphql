...
import COMPONENT_IMAGE from "./graphql-mutate.png";
...
export default class GraphqlMutate extends DataSource(RectPath(Shape)) {
  static get image() {
    if (!GraphqlMutate._image) {
      GraphqlMutate._image = new Image();
      GraphqlMutate._image.src = COMPONENT_IMAGE;
    }

    return GraphqlMutate._image;
  }
  ...
  ready() {
    ...
    this._initGraphqlMutateConnection();
  }

  _initGraphqlMutateConnection() {
    try {
  ...
  render(context) {
    ...
    context.beginPath();
    context.drawImage(GraphqlMutate.image, left, top, width, height);
  }
...
Component.register("graphql-mutate", GraphqlMutate);
